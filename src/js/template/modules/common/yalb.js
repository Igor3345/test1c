    ...
    var supportsTransitions = 'transition' in document.createElement('p').style,
        ...
        yalb = function (list, options) {
            ...
            hideShowButtons = function () {
                if (settings.loop && list.length > 1) {
                    return;
                }

                if (current === list.length - 1) {
                    addClass(container.querySelector('.next'), 'hide');
                } else {
                    removeClass(container.querySelector('.next'), 'hide');
                }

                if (current === 0) {
                    addClass(container.querySelector('.prev'), 'hide');
                } else {
                    removeClass(container.querySelector('.prev'), 'hide');
                }
            };
            ...
            // shows the image
            showImg = function (e) {
                if (e && e.target !== container && e.propertyName !== 'width') {
                    return;
                }
                ...
                img.addEventListener('transitionend', function () {
                    if (window.getComputedStyle(img).getPropertyValue('opacity') == 0 && !!img.closest('.yalb')) {
            ...
            loadImg = function (arr) {
                ...
                for (var i = arr.length; i--;) {
                    ...
                    if (arr[i] >= images.length || arr[i] < 0 || images[arr[i]].loaded || images[arr[i]].pending || images[arr[i]].error) {
                        continue;
                    }
            ...
            resizeWindow = function () {
                ...
                if (container.querySelector('.image') && container.querySelector('.image').src === images[current].img.src) {
                    return;
                }
